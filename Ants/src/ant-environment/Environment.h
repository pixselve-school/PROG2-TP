    ...
    void rasterize(LocalizedEntity *entity, const Function &function) {
        ...
        for (unsigned int x = minX; x <= maxX; ++x) {
            for (unsigned int y = minY; y < maxY; ++y) {
                Rectangle cellShape(Vector2<float>((float)x,(float)
                y), Vector2<float>(1.0f, 1.0f));
...
public:
    ...
    ::std::vector<T *> perceive(const Vector2<unsigned int> &cell) const {
        assert(isValid(cell) &&
               "Environment::perceive: invalid cell provided. Please, use Environment::cell to convert your coordinates");
    ...
    template<class T>
    ::std::vector<T *>
    perceive(Vector2<float> const &position, const Vector2<float> &direction, float openingAngle, float extent,
