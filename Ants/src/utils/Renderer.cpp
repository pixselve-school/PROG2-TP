...
void Renderer::flushStrings() {
    for (auto it = m_strings.begin(), end = m_strings.end(); it != end; ++it) {
        stringRGBA(getSdlRenderer(), (Sint16) it->position[0], (Sint16) it->position[1], it->string.c_str(),
...
void Renderer::flushCircles() {
    for (auto it = m_circles.begin(), end = m_circles.end(); it != end; ++it) {
        filledCircleRGBA(getSdlRenderer(), (Sint16) it->center[0], (Sint16) it->center[1], (Sint16) it->radius,
                         it->color[0], it->color[1], it->color[2], it->color[3]);
    }
